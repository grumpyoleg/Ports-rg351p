...
if len(sys.argv) < 2:
    print(f'Error: No settings path specified')
    _exit(1)
else:
    Settings.DATA_DIR = sys.argv[1]
...
def load_config():
    ...
    if not Path(str(Settings.DATA_DIR) + '/Settings.cfg').is_file():
        ...
        poolservice = config["AVR Miner"]["pool_service"]
        miner_ip = config["AVR Miner"]["miner_ip"]
        miner_port = config["AVR Miner"]["miner_port"]
        devices = config["AVR Miner"]["devices"]
        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    msg_send = str(job[0] + Settings.SEPARATOR + job[1]
                                   + Settings.SEPARATOR
                                   + job[2]
                                   + Settings.SEPARATOR)

                    avrsock = socket(AF_INET, SOCK_STREAM)
                    avrsock.connect((miner_ip, int(miner_port)))
                    encoded_string = str.encode(msg_send + '\n')
                    avrsock.sendall(bytearray(encoded_string))
                    res2 = avrsock.recv(1024)
                    res2 = res2.decode().strip()
                    avrsock.close()
...
if __name__ == '__main__':
    ...
    try:
        ...
        threadid = 0
        nanos = 0
        for nanos in range(int(devices)):
            thrThread(target=mine_avr,
                      args=(str(avrport), threadid,
                            fastest_pool)).start()
            threadid += 1
    except Exception as e:
        print(f'Error launching AVR thread(s): {e}')
